namespace FtrIO.Strategies
{
    ...
    public class PercentageRolloutStrategy : IToggleDecisionStrategy
    {
        ...
        {
            var percentageText = rawValue.TrimEnd('%').Trim();
            if (!double.TryParse(percentageText, NumberStyles.Number, CultureInfo.InvariantCulture, out var rolloutPercentage)
                || rolloutPercentage < 0 || rolloutPercentage > 100)
                throw new ToggleParsedOutOfRangeException();

            return Random.Shared.NextDouble() * 100 < rolloutPercentage;
        }
    }
}
